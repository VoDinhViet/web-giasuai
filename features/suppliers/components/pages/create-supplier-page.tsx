...
import { useSupplierCreateDraft } from "../../hooks/use-supplier-create-draft"
import {
  supplierCreatePageSchema,
...
const defaultSupplierValues: SupplierCreatePageInput = {
  supplierCode: "",
  logoFile: null,
  name: "",
  supplierGroupId: "",
...
export function CreateSupplierPage({
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: supplierCreatePageSchema,
    },
    onSubmit: async ({ value }) => {
      setFormFeedback(null)

      const createSupplierResult = await createSupplier({
        name: value.name,
      ...
      if (!createSupplierResult.success) {
        setFormFeedback({
          type: "error",
          message: createSupplierResult.message,
        })
        return
      }
  ...
  const { clearDraft, saveDraft } = useSupplierCreateDraft({
    defaultValues: defaultSupplierValues,
    form,
  })

  function handleSaveDraft(value: SupplierCreatePageInput) {
    saveDraft(value)
    setFormFeedback({
      type: "success",
      message: "Đã lưu nháp trên trình duyệt.",
    })
  }
                ...
                return (
                  <Field data-invalid={isInvalid} className="xl:col-span-2">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        field.handleChange(
                          value as SupplierCreatePageInput["supplierType"]
                        )
                      }
                    >
