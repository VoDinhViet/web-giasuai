...
import {
  classAdmissionModeOptions,
  classLearningModeOptions,
  classStatusOptions,
  classWeekdayOptions,
  createClassSchema,
...
const createClassDefaultValues: CreateClassInput = {
  code: "",
  name: "",
  courseId: undefined,
  teacherId: undefined,
  ...
  admissionMode: "REQUEST_APPROVAL",
  allowWaitlist: true,
  sendReminder: true,
...
export function CreateClassForm({
  ...
  const teacherLabelById = useMemo(
    () => new Map(teacherOptions.map((option) => [option.value, option.label])),
    [teacherOptions]
  )
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (!createClassResult.success) {
        setSubmitError(createClassResult.message)
        return
      }
    ...
    >
      <div className="min-w-0 space-y-5">
        ...
        <FormSection
          ...
          description="Gắn khóa chính và phân công giáo viên quản lý lớp."
        >
          <FieldGroup className="grid gap-5 sm:grid-cols-2">
          ...
          <FieldGroup className="grid gap-5 sm:grid-cols-2">
                ...
                return (
                  <Field data-invalid={isInvalid} className="sm:col-span-2">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        field.handleChange(value as ClassLearningMode)
                      }
                    >
                      {classLearningModeOptions.map((modeOption) => (
                        <RadioOption
            ...
            <form.Field name="room">
              {(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    <FieldLabel htmlFor={field.name}>Phòng học</FieldLabel>
                    <Input
                      id={field.name}
                      name={field.name}
                      value={field.state.value}
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    ...
                    <Select
                      value={field.state.value}
                      onValueChange={(value) =>
                        field.handleChange(value as ClassAdmissionMode)
                      }
                    >
                      <SelectTrigger
                        className="w-full"
                      ...
                      <SelectContent>
                        {classAdmissionModeOptions.map((modeOption) => (
                          <SelectItem
                            key={modeOption.value}
                            value={modeOption.value}
                          >
      ...
      <form.Subscribe selector={(state) => state.values}>
        {(values) => (
          <ClassPreview
            values={values}
            courseLabel={
              values.courseId ? courseLabelById.get(values.courseId) : undefined
            }
            teacherLabel={
              values.teacherId
                ? teacherLabelById.get(values.teacherId)
                : undefined
            }
          />
        )}
...
function ClassPreview({
  ...
  const learningModeLabel = getOptionLabel(
    classLearningModeOptions,
    values.learningMode
  )
  const admissionModeLabel = getOptionLabel(
    classAdmissionModeOptions,
    values.admissionMode
  )
      ...
      <section className="rounded border border-border/80 bg-card p-5 shadow-xs">
        ...
        <div className="grid gap-4">
          ...
          <PreviewRow
            ...
            value={formatDateRange(values.startDate, values.endDate)}
          />
          <PreviewRow
            icon={MapPin}
            label="Địa điểm"
            value={values.room || "Chưa có phòng học"}
          />
          <PreviewRow
            icon={Monitor}
            label="Hình thức"
...
function formatAutomationSummary(values: CreateClassInput) {
  const enabledAutomations = [
    values.allowWaitlist ? "Danh sách chờ" : undefined,
    values.sendReminder ? "Nhắc lịch" : undefined,
